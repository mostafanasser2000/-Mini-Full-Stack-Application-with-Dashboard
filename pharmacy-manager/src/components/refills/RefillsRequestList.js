...
const RefillRequestList = () => {
  ...
  async function handleCancelRequest(id) {
    const previousRequests = [...refillRequests];
    try {
      setLoading(true);
      const updatedRequests = refillRequests.filter(
      ...
      setRefillRequests(previousRequests);
      setStats(calculateStats(previousRequests));
      setError(err.message);
    } finally {
      setLoading(false);
    }
